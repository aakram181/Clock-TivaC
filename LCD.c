...
void LCDCMD(unsigned char cmd) {
		GPIOPinWrite(GPIO_PORTD_BASE,REGISTER_SELECT|READ_WRITE,!REGISTER_SELECT|!READ_WRITE); // RS= 0 , RW = 0
		GPIOPinWrite(GPIO_PORTB_BASE,D0|D1|D2|D3|D4|D5|D6|D7,cmd); // write command
		GPIOPinWrite(GPIO_PORTD_BASE,ENABLE,ENABLE); // E = 1
    SysCtlDelay(10);
    GPIOPinWrite(GPIO_PORTD_BASE,ENABLE,!ENABLE); // E = 0
    SysCtlDelay(5000);

}

void LCDPrintChar (unsigned char data) {
		GPIOPinWrite(GPIO_PORTD_BASE,REGISTER_SELECT|READ_WRITE,REGISTER_SELECT|!READ_WRITE); // RS= 1, RW = 0
		GPIOPinWrite(GPIO_PORTB_BASE,D0|D1|D2|D3|D4|D5|D6|D7,data); //write text
		GPIOPinWrite(GPIO_PORTD_BASE,ENABLE,ENABLE); // E = 1
    SysCtlDelay(10);
		GPIOPinWrite(GPIO_PORTD_BASE,ENABLE|REGISTER_SELECT,!ENABLE|!REGISTER_SELECT); // E = 0
    SysCtlDelay(500);
...
void LCDInit(){
  ...
  GPIOPinTypeGPIOOutput(GPIO_PORTD_BASE, (REGISTER_SELECT|READ_WRITE|ENABLE));
